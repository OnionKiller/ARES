...
for evaluation_dataset in evaluation_datasets:
    ...
    if not use_annotations_for_ranking:
        ...
        context_subset = dataset.dropna(subset=["Context_Relevance_Label"]).sample(n=300, random_state=42) 
        answer_subset = dataset.dropna(subset=["Answer_Relevance_Label"]).sample(n=300, random_state=42) 
        print("context_subset: " + str(len(context_subset)))
        print("answer_subset: " + str(len(answer_subset)))
        context_subset = context_subset.sample(n=300, random_state=42) 
        answer_subset = answer_subset.sample(n=300, random_state=42) 
